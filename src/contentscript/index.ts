...
let aiSummarizer = document.querySelector<HTMLDivElement>(
  "#gitlab-ai-summarizer"
);
if (!aiSummarizer) {
  aiSummarizer = document.createElement("div");
...
let issueDetails = document.querySelector<HTMLDivElement>(
  "#gitlab-ai-summarizer_details"
);
if (!issueDetails) {
  issueDetails = document.createElement("div");
...
if (!headerRanbotLogo) {
  headerRanbotLogo = document.createElement("img");
  headerRanbotLogo.id = "gitlab-ai-summarizer_header-ranbot-logo";
}
headerRanbotLogo.src = defaultLogoURL;
headerRanbotLogo.alt = defaultTitle;
headerRanbotLogo.style.float = "left";

let headerCloseIcon = document.querySelector<HTMLSpanElement>(
  "#gitlab-ai-summarizer_header-close-icon"
);
if (!headerCloseIcon) {
  headerCloseIcon = document.createElement("span");
  headerCloseIcon.id = "gitlab-ai-summarizer_header-close-icon";
}
...
let headerSettingIcon = document.querySelector<HTMLSpanElement>(
  "#gitlab-ai-summarizer_header-setting"
);
if (!headerSettingIcon) {
  headerSettingIcon = document.createElement("span");
  headerSettingIcon.id = "gitlab-ai-summarizer_header-setting";
}
...
let headerCopyIcon = document.querySelector<HTMLSpanElement>(
  "#gitlab-ai-summarizer_header-copy"
);
if (!headerCopyIcon) {
  headerCopyIcon = document.createElement("span");
  headerCopyIcon.id = "gitlab-ai-summarizer_header-copy";
}
...
let progressInfo = document.querySelector<HTMLSpanElement>(
  "#gitlab-ai-summarizer_header-progress"
);
if (!progressInfo) {
  progressInfo = document.createElement("span");
  progressInfo.id = "gitlab-ai-summarizer_header-progress";
}
...
let versionNumber = document.querySelector<HTMLDivElement>(
  "#gitlab-ai-summarizer_footer-version-number"
);
if (!versionNumber) {
  versionNumber = document.createElement("div");
  versionNumber.id = "gitlab-ai-summarizer_footer-version-number";
} else {
...
const closeEvent = () => {
  ...
  if (versionNumber) versionNumber.style.display = "none";

  if (headerRanbotLogo)
    headerRanbotLogo.removeEventListener("click", aiSummarizerEvent, false);
  if (issueDetails) issueDetails.remove();

  setupRanBOT();
};

const copyIconEvent = () => {
  if (issueDetails) navigator.clipboard.writeText(issueDetails.innerText);
  if (headerCopyIcon)
...
const aiSummarizerEvent = async () => {
  ...
  if (startGitLabAPI) {
    ...
    if (projectPath === undefined && issueId === undefined && progressInfo) {
      progressInfo.innerText = `This is not a GitLab URL.`;
      progressInfo.style.fontSize = "1.2rem";
    } else if (projectPath === undefined && progressInfo) {
      progressInfo.innerText = `Project '${projectPath}' not found.`;
      progressInfo.style.fontSize = "1.1rem";
    } else if (issueId === undefined && progressInfo) {
      ...
      let age = document.querySelector<HTMLParagraphElement>(
        "#gitlab-ai-summarizer_details-age"
      );
      if (!age) {
        age = document.createElement("p");
        age.id = "gitlab-ai-summarizer_details-age";
      } else {
        age.remove();
      }
      age.innerHTML = `<b>Age:</b> ${ticketAge} days. <em>${createdAt}</em>`;
      setupBlockStyle(age);
      if (issueDetails) issueDetails.appendChild(age);

      let lastUpdated = document.querySelector<HTMLParagraphElement>(
        "#gitlab-ai-summarizer_details-last-updated"
      );
      if (!lastUpdated) {
        lastUpdated = document.createElement("p");
        lastUpdated.id = "gitlab-ai-summarizer_details-last-updated";
      } else {
        lastUpdated.remove();
      }
      ...
      if (issueDetails) issueDetails.appendChild(lastUpdated);

      let comments = document.querySelector<HTMLParagraphElement>(
        "#gitlab-ai-summarizer_details-comments"
      );
      if (!comments) {
        comments = document.createElement("p");
        comments.id = "gitlab-ai-summarizer_details-comments";
      } else {
        comments.remove();
      }
      comments.innerHTML = `<b>Comments:</b> ${issueData.user_notes_count}`;
      setupBlockStyle(comments);
      if (issueDetails) issueDetails.appendChild(comments);
      ...
      let lastBranch = document.querySelector<HTMLParagraphElement>(
        "#gitlab-ai-summarizer_details-last-branch"
      );
      if (!lastBranch) {
        lastBranch = document.createElement("p");
        lastBranch.id = "gitlab-ai-summarizer_details-last-branch";
      } else {
        lastBranch.remove();
      }
      ...
      if (issueDetails) issueDetails.appendChild(lastBranch);
      if (aiSummarizer && issueDetails) aiSummarizer.appendChild(issueDetails);
    ...
    if (progressInfo) progressInfo.innerText = `GitLab AI Summarizer`;
    if (versionNumber && aiSummarizer) aiSummarizer.appendChild(versionNumber);
  }
};
...
const setupRanBOT = () => {
  ...
  if (header && headerRanbotLogo && aiSummarizer) {
    header.appendChild(headerRanbotLogo);

    aiSummarizer.appendChild(header);
  }
  if (aiSummarizer) document.body.appendChild(aiSummarizer);
};
